...
use filigree_htmx_test_app::{cmd, emails, server, Error};
...
async fn serve(cmd: ServeCommand) -> Result<(), Report<Error>> {
    ...
    filigree_htmx_test_app::db::run_migrations(&pg_pool).await?;
    ...
    let server = server::create_server(server::Config {
        ...
        storage: filigree_htmx_test_app::storage::AppStorageConfig::new()
            .change_context(Error::ServerStart)?,
    })
    .await?;

...
use super::{
    OrganizationId, CREATE_PERMISSION, OWNER_PERMISSION, READ_PERMISSION, WRITE_PERMISSION,
};
...
impl<'a> AuthedEntity<'a> {
    ...
    /// Defines a "belongs to" relationship between the Organization and the related entity.
    ...
    /// Defines a "has one" relationship between the Organization and the related entity.
    ...
    /// Defines a "has many" relationship between the Organization and the related entity.
    ...
    /// Performs an unauthenticated find operation on Organizations.
    pub fn find_unauthed() -> Select<Entity> {
        Entity::find()
    }

    /// Performs an unauthenticated find operation by ID on Organization.
    ...
    /// Performs an unauthenticated insert operation for a single Organization.
    ...
    /// Performs an unauthenticated insert operation for multiple Organizations.
    ...
    /// Performs an unauthenticated update operation for a single Organization.
    ...
    /// Performs an unauthenticated update operation for multiple Organizations.
    pub fn update_many_unauthed() -> sea_orm::UpdateMany<Entity> {
        Entity::update_many()
    }

    /// Performs an unauthenticated delete operation for a single Organization.
    ...
    /// Performs an unauthenticated delete operation for multiple Organizations.
    ...
    pub fn get_access(&self, id: &OrganizationId) -> ObjectPermission {

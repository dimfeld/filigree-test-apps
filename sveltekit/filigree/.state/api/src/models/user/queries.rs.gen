...
use super::{types::*, ActiveModel, BaseEntity, Column, Entity, UserId};
...
/*
/// Get a User from the database or return a `NotFound` error.
#[instrument(skip(db))]
pub async fn get(db: impl ConnectionTrait, auth: &AuthInfo, id: UserId) -> Result<Model, error_stack::Report<Error>> {
...
pub async fn list(
    ...
    auth: &AuthInfo,
    filters: ListQueryFilters) -> Result<Vec<UserListResult>, error_stack::Report<Error>> {
...
pub async fn create(db: &mut PgConnection, auth: &AuthInfo, payload: UserCreatePayload) -> Result<UserCreateResult, error_stack::Report<Error>> {
...
pub async fn create_raw(
    ...
    let result = query_file_as!(User, "src/models/user/insert.sql",
        id.as_uuid(),
        organization_id.as_uuid(),
        &payload.name,payload.email.as_ref(),payload.avatar_url.as_ref(),
        )
        .fetch_one(&mut *db)
...
-> Result<bool, error_stack::Report<Error>> {
    let actor_ids = auth.actor_ids();
    let result = query_file_scalar!("src/models/user/update.sql",
        id.as_uuid(),
        auth.organization_id.as_uuid(),
        &actor_ids,
        &payload.name as _,payload.email.as_ref() as _,payload.avatar_url.as_ref() as _,
        )
...
#[instrument(skip(db))]
pub async fn delete(db: impl ConnectionTrait, auth: &AuthInfo, id: &UserId) -> Result<bool, error_stack::Report<Error>> {
    let actor_ids = auth.actor_ids();
    let result = query_file!("src/models/user/delete.sql",
        id.as_uuid(),
        auth.organization_id.as_uuid(),
        &actor_ids
        )

...
impl Tag {
    ...
    ) -> Result<Vec<TagListResult>, error_stack::Report<Error>> {
    ...
    ) -> Result<TagCreateResult, error_stack::Report<Error>> {
    ...
    /// Create a new Tag in the database, allowing the ID to be explicitly specified
    ...
    pub async fn lookup_object_permissions(
        db: impl PgExecutor<'_>,
        auth: &AuthInfo,
        #[allow(unused_variables)] id: &TagId,

...
use super::{PostImageId, CREATE_PERMISSION, OWNER_PERMISSION, READ_PERMISSION, WRITE_PERMISSION};
use crate::{
    auth::AuthInfo,
    error::Error,
...
pub enum Relation {
    #[sea_orm(
        belongs_to = "crate::models::post::entity::Entity",
        from = "Column::PostId",
        to = "crate::models::post::Column::Id"
    )]
    Post,
}

impl Related<crate::models::post::entity::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::Post.def()
    }
}
...
impl<'a> AuthedEntity<'a> {
    ...
    /// Defines a "belongs to" relationship between the PostImage and the related entity.
    ...
    /// Defines a "has one" relationship between the PostImage and the related entity.
    ...
    /// Defines a "has many" relationship between the PostImage and the related entity.
    ...
    /// Performs an authenticated find operation on PostImage
    pub fn find(&self) -> Result<Select<Entity>, error_stack::Report<Error>> {
        self.check_list(Entity::find())
    }

    /// Performs an authenticated find operation by ID on PostImage
    ...
    /// Performs an authenticated insert operation for a single PostImage.
    ...
    /// Performs an authenticated insert operation for multiple PostImages.
    ...
    /// Performs an authenticated update operation for a single PostImage.
    ...
    /// Performs an authenticated update operation for multiple PostImages.
    ...
    /// Performs an authenticated delete operation for a single PostImage.
    ...
    /// Performs an authenticated delete operation for multiple PostImages.
    ...
    /// Performs an unauthenticated find operation on PostImages.
    pub fn find_unauthed() -> Select<Entity> {
        Entity::find()
    }

    /// Performs an unauthenticated find operation by ID on PostImage.
    ...
    /// Performs an unauthenticated insert operation for a single PostImage.
    ...
    /// Performs an unauthenticated insert operation for multiple PostImages.
    ...
    /// Performs an unauthenticated update operation for a single PostImage.
    ...
    /// Performs an unauthenticated update operation for multiple PostImages.
    pub fn update_many_unauthed() -> sea_orm::UpdateMany<Entity> {
        Entity::update_many()
    }

    /// Performs an unauthenticated delete operation for a single PostImage.
    ...
    /// Performs an unauthenticated delete operation for multiple PostImages.
    ...
    pub fn get_access(&self, id: &PostImageId) -> ObjectPermission {

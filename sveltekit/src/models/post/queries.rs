...
use super::{types::*, ActiveModel, BaseEntity, Column, Entity, PostId};
use crate::{
    auth::AuthInfo,
    models::{
...
/*
/// Get a Post from the database or return a `NotFound` error.
#[instrument(skip(db))]
pub async fn get(db: impl ConnectionTrait, auth: &AuthInfo, id: PostId) -> Result<Model, error_stack::Report<Error>> {
...
#[instrument(skip(db))]
pub async fn get_populated(db: impl ConnectionTrait, auth: &AuthInfo, id: &PostId) -> Result<PostPopulatedGetResult, error_stack::Report<Error>> {
    let actor_ids = auth.actor_ids();
    let object = query_file_as!(PostPopulatedGetResult, "src/models/post/select_one_populated.sql",
        id.as_uuid(),
...
pub async fn list(
    ...
    auth: &AuthInfo,
    filters: ListQueryFilters) -> Result<Vec<PostListResult>, error_stack::Report<Error>> {
...
pub async fn list_populated(
    db: impl ConnectionTrait,
    auth: &AuthInfo,
    filters: ListQueryFilters) -> Result<Vec<PostPopulatedListResult>, error_stack::Report<Error>> {
...
pub async fn create(db: &mut PgConnection, auth: &AuthInfo, payload: PostCreatePayload) -> Result<PostCreateResult, error_stack::Report<Error>> {
...
pub async fn create_raw(
    ...
    let result = query_file_as!(Post, "src/models/post/insert.sql",
        id.as_uuid(),
...
-> Result<bool, error_stack::Report<Error>> {
    let actor_ids = auth.actor_ids();
    let result = query_file_scalar!("src/models/post/update.sql",
        id.as_uuid(),
        auth.organization_id.as_uuid(),
        &actor_ids,
        &payload.subject as _,&payload.body as _,
        )
...
#[instrument(skip(db))]
pub async fn delete(db: impl ConnectionTrait, auth: &AuthInfo, id: &PostId) -> Result<bool, error_stack::Report<Error>> {
    let actor_ids = auth.actor_ids();
    let result = query_file!("src/models/post/delete.sql",
        id.as_uuid(),
        auth.organization_id.as_uuid(),
        &actor_ids
        )

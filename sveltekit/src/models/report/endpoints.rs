...
async fn get(
    ...
    let object = Report::get_populated(&state.db, &auth, &id).await?;
...
async fn list(
    ...
    let results = Report::list_populated(&state.db, &auth, &qs).await?;
...
async fn create(
    ...
    let result = Report::create(&mut *tx, &auth, payload).await?;
...
async fn update(
    ...
    let result = Report::update(&mut *tx, &auth, &id, payload).await?;
...
async fn delete(
    ...
    let deleted = Report::delete(&mut *tx, &auth, &id).await?;
...
async fn list_child_report_section(
    ...
    let object = crate::models::report_section::ReportSection::list(&state.db, &auth, &qs).await?;
...
async fn get_child_report_section(
    ...
    let object =
        crate::models::report_section::ReportSection::get(&state.db, &auth, &child_id).await?;
...
async fn create_child_report_section(
    ...
    let result =
        crate::models::report_section::ReportSection::create(&mut *tx, &auth, payload).await?;
...
async fn update_child_report_section(
    ...
    let object_perm = Report::lookup_object_permissions(&state.db, &auth, &parent_id)
        .await?
...
mod test {
    ...
    async fn setup_test_objects(
        ...
        for i in 0..count {
            ...
            let result = Report::create_raw(&mut *tx, &id, &organization_id, payload.clone())
                .await
    ...
    async fn list_objects(pool: sqlx::PgPool) {
        ...
        for result in results {
            ...
            assert_eq!(result["tag_ids"], ids, "field tag_ids");
        }
    ...
    async fn get_object(pool: sqlx::PgPool) {
        ...
        assert_eq!(
            result["tags"],
            serde_json::to_value(&added.tags).unwrap(),
            "field tags"
        );
    ...
    async fn child_tag(pool: sqlx::PgPool) {
        ...
        let create_result_one = admin_user
            .client
            .post(&format!("reports/{}/tags", parent_result.id))
            .json(&payload_one)
            .send()
            .await
            .unwrap()
        ...
        let response = admin_user
            .client
            .post(&format!("reports/{}/tags", bad_parent_id))
            .json(&payload_two)
            .send()
            .await
            .unwrap();
        ...
        // Create another test object
        let payload_two = crate::models::tag::testing::make_create_payload(2);
        let create_result_two = admin_user
            .client
            .post(&format!("reports/{}/tags", parent_result.id))
            .json(&payload_two)
            ...
            .await
            .unwrap();
        let id_two = TagId::from_str(create_result_two["id"].as_str().unwrap()).unwrap();
        ...
        let res = no_roles_user
            .client
            .get(&format!("reports/{}/tags/{}", parent_result.id, id_one))
            .send()
            .await
            .unwrap();
        assert_eq!(res.status(), reqwest::StatusCode::FORBIDDEN);
        ...
        let res = no_roles_user
            .client
            .put(&format!("reports/{}/tags/{}", parent_result.id, id_one))
            .json(&bad_update_payload)
            .send()
            .await
            .unwrap();
        ...
        let res = no_roles_user
            .client
            .delete(&format!("reports/{}/tags/{}", parent_result.id, id_one))
            .send()
            .await
            .unwrap();
        assert_eq!(res.status(), reqwest::StatusCode::FORBIDDEN);
        ...
        let list_result = admin_user
            .client
            .get(&format!("reports/{}/tags", parent_result.id))
            .send()
            .await
            .unwrap()
            .log_error()

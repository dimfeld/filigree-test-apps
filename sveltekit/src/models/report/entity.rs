...
use super::{ReportId, CREATE_PERMISSION, OWNER_PERMISSION, READ_PERMISSION, WRITE_PERMISSION};
use crate::{
    auth::AuthInfo,
    error::Error,
    models::{
...
pub enum Relation {
    #[sea_orm(has_many = "crate::models::report_section::entity::Entity")]
    ReportSection,
}

impl Related<crate::models::report_section::entity::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::ReportSection.def()
    }
}
...
impl<'a> AuthedEntity<'a> {
    ...
    /// Defines a "belongs to" relationship between the Report and the related entity.
    ...
    /// Defines a "has one" relationship between the Report and the related entity.
    ...
    /// Defines a "has many" relationship between the Report and the related entity.
    ...
    /// Performs an authenticated find operation on Report
    pub fn find(&self) -> Result<Select<Entity>, error_stack::Report<Error>> {
        self.check_list(Entity::find())
    }

    /// Performs an authenticated find operation by ID on Report
    ...
    /// Performs an authenticated insert operation for a single Report.
    ...
    /// Performs an authenticated insert operation for multiple Reports.
    ...
    /// Performs an authenticated update operation for a single Report.
    ...
    /// Performs an authenticated update operation for multiple Reports.
    ...
    /// Performs an authenticated delete operation for a single Report.
    ...
    /// Performs an authenticated delete operation for multiple Reports.
    ...
    /// Performs an unauthenticated find operation on Reports.
    pub fn find_unauthed() -> Select<Entity> {
        Entity::find()
    }

    /// Performs an unauthenticated find operation by ID on Report.
    ...
    /// Performs an unauthenticated insert operation for a single Report.
    ...
    /// Performs an unauthenticated insert operation for multiple Reports.
    ...
    /// Performs an unauthenticated update operation for a single Report.
    ...
    /// Performs an unauthenticated update operation for multiple Reports.
    pub fn update_many_unauthed() -> sea_orm::UpdateMany<Entity> {
        Entity::update_many()
    }

    /// Performs an unauthenticated delete operation for a single Report.
    ...
    /// Performs an unauthenticated delete operation for multiple Reports.
    ...
    pub fn get_access(&self, id: &ReportId) -> ObjectPermission {

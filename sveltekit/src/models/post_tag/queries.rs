...
use crate::{
    auth::AuthInfo,
    models::{organization::OrganizationId, post::PostId, tag::TagId},
    Error,
};
...
impl PostTag {
    fn check_missing_parent_error<T>(
        ...
        match result {
            Err(sqlx::Error::Database(e)) if e.constraint() == Some("post_tags_post_id_fkey") => {
                Err(e).change_context(Error::NotFound("Parent post_id"))
            }

            Err(sqlx::Error::Database(e)) if e.constraint() == Some("post_tags_tag_id_fkey") => {
    ...
    /// Get a PostTag from the database
    #[instrument(skip(db))]
    pub async fn get(
        db: impl PgExecutor<'_>,
        auth: &AuthInfo,
        id: &(PostId, TagId),
    ) -> Result<PostTag, error_stack::Report<Error>> {
        auth.require_permission(super::READ_PERMISSION)?;

        let object = query_file_as!(
            PostTag,
            "src/models/post_tag/select_one.sql",
    ...
    ) -> Result<Vec<PostTagListResult>, error_stack::Report<Error>> {
    ...
    pub async fn create(
        db: &mut PgConnection,
        auth: &AuthInfo,
        payload: PostTagCreatePayload,
    ) -> Result<PostTagCreateResult, error_stack::Report<Error>> {
        auth.require_permission(super::CREATE_PERMISSION)?;

        let id = (
            payload.post_id.clone().ok_or(Error::MissingId("post_id"))?,
    ...
    /// Create a new PostTag in the database, allowing the ID to be explicitly specified
    ...
    pub async fn create_raw(
        ...
        let result = query_file_as!(
            PostTag,
            "src/models/post_tag/insert.sql",
            id.0.as_uuid(),
            id.1.as_uuid(),
    ...
    pub async fn delete(
        ...
        let result = query_file!(
            "src/models/post_tag/delete.sql",
            id.0.as_uuid(),
            id.1.as_uuid(),
            auth.organization_id.as_uuid()
        )
    ...
    pub async fn lookup_object_permissions(
        db: impl PgExecutor<'_>,
        auth: &AuthInfo,
        #[allow(unused_variables)] id: &(PostId, TagId),
    ...
    pub async fn upsert_with_parent_post(
        ...
        let result = query_file_as!(
            PostTag,
            "src/models/post_tag/upsert_single_child_of_post.sql",
            id.0.as_uuid(),
            id.1.as_uuid(),
            organization_id.as_uuid(),
            parent_id.as_uuid()
    ...
    pub async fn delete_with_parent_post(
        ...
        let result = query_file!(
            "src/models/post_tag/delete_with_parent_post.sql",
            auth.organization_id.as_uuid(),
            parent_id.as_uuid(),
            id.0.as_uuid(),
            id.1.as_uuid()
    ...
    pub async fn delete_all_children_of_post(
        ...
        let result = query_file!(
            "src/models/post_tag/delete_all_children_of_post.sql",
            organization_id.as_uuid(),
            parent_id.as_uuid()
        )
        .execute(db)
    ...
    pub async fn upsert_with_parent_tag(
        ...
        let result = query_file_as!(
            PostTag,
            "src/models/post_tag/upsert_single_child_of_tag.sql",
            id.0.as_uuid(),
            id.1.as_uuid(),
            organization_id.as_uuid()
        )
    ...
    pub async fn update_all_with_parent_tag(
        ...
        parent_id: &TagId,
        payload: &[PostTagUpdatePayload],
    ) -> Result<Vec<PostTag>, error_stack::Report<Error>> {
        if payload.is_empty() {
            ...
            let mut query = sqlx::query_as::<_, PostTag>(q.as_str())
                .bind(organization_id)
                .bind(parent_id);

            for p in payload {
                let post_id = p.post_id.as_ref().ok_or(Error::MissingId("post_id"))?;
                let tag_id = parent_id;

                query = query.bind(post_id).bind(tag_id).bind(organization_id)
            }
            ...
            let ids = results
                .iter()
                .map(|o| o.post_id.as_uuid().clone())
                .collect::<Vec<_>>();

            query_file!(
                "src/models/post_tag/delete_removed_children_of_tag.sql",
                organization_id.as_uuid(),
                parent_id.as_uuid(),
                &ids
            )
    ...
    pub async fn delete_with_parent_tag(
        ...
        let result = query_file!(
            "src/models/post_tag/delete_with_parent_tag.sql",
            auth.organization_id.as_uuid(),
            parent_id.as_uuid(),
            id.0.as_uuid(),
            id.1.as_uuid()
    ...
    pub async fn delete_all_children_of_tag(
        ...
        let result = query_file!(
            "src/models/post_tag/delete_all_children_of_tag.sql",
            organization_id.as_uuid(),
            parent_id.as_uuid()
        )
        .execute(db)
